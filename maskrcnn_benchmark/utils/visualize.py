...
def vis_image(img, boxes, cls_prob=None, mode=2, line_width=2):
    ...
    for idx in range(len(boxes)):
        ...
        if mode == 1:
            draw.rectangle(pts ,outline=(0, 255, 0), width=line_width)

        elif mode == 0:
            draw.rectangle(pts ,outline=(0, 0, 255), width=line_width)

        elif mode == 2:
            draw.rectangle(pts ,outline=(255, 255, 0), width=line_width)
    ...
    for idx in range(len(dets)):
        ...
        if not voc_list is None:
            ...
            for i_voc in range(len(voc_list)):
                dis = editdistance.eval(res_word.upper(), voc_list[i_voc].replace(
                    '\ufeff', '').replace('\n', '').upper())
                edisarr.append(dis)
                if dis < min_dis:
                    min_dis = dis
